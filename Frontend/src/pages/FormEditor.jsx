                ...
                {editingId === field.id ? (
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                      <input type="checkbox" checked={editData.required || false} onChange={(e) => setEditData({ ...editData, required: e.target.checked })} className="w-5 h-5" />
                      <label className="text-sm font-semibold text-gray-700">Obrigatório</label>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => saveField(field.id)} className="px-4 py-2 bg-blue-600 text-white rounded-lg">Guardar</button>
                      <button onClick={cancelEditing} className="px-4 py-2 bg-gray-400 text-white rounded-lg">Cancelar</button>
                    </div>
                  </div>
                ) : (
                  <>
                    <div className="mb-3 flex justify-between items-center">
                      <span className="text-xs font-semibold text-gray-500 uppercase">{field.type} {field.required && <span className="text-red-500">*</span>}</span>
                      <div className="flex gap-2">
