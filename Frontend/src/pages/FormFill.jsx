...
// Página de preenchimento de formulário publicado (#90, #91)
// ======================================================
...
// ======================================================
// RENDERIZAÇÃO INTERATIVA DE CAMPOS
// ======================================================

const FillLabel = ({ field }) => (
  <div className="mb-6 p-3 border-l-4 border-indigo-500 bg-indigo-50 rounded-r-lg">
    <h3 className="text-lg font-bold text-indigo-900">{field.value || 'Secção'}</h3>
    {field.description && (
      <p className="text-sm text-indigo-700 mt-1">{field.description}</p>
    )}
  </div>
);
  ...
  <div className="mb-6">
    ...
    <div className="space-y-2">
      {(field.options || []).map((opt, i) => (
        <label
          key={i}
          className="flex items-center p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-indigo-50 hover:border-indigo-300 transition"
        >
          <input
            type="radio"
            name={field.id}
...
const FillDropdown = ({ field, value, onChange }) => (
  <div className="mb-6">
    ...
    <select
      ...
      className="w-full p-3 border border-gray-300 rounded-lg bg-white text-gray-700 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition"
    >
...
const renderField = (field, formData, handleChange) => {
  switch (field.type) {
    case 'label':
      return <FillLabel key={field.id} field={field} />;
    case 'radio':
      return (
        <FillRadio
          key={field.id}
...
const FormFill = ({ formId, authUser, authToken, onGoHome }) => {
    ...
    const loadTemplate = async () => {
      try {
        ...
        if (data.status !== 'published') {
          throw new Error('Este formulário não está disponível para preenchimento.');
        }
  ...
  // Submeter formulário (#91 — associar ao utilizador autenticado)
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        data: formData,
        submitted_by: authUser?.id ?? null, // #91 — utilizador autenticado
      };
      ...
      const response = await fetch(`${API_BASE}/form-submissions`, {
        ...
        headers: {
          ...
          Authorization: `Bearer ${authToken}`, // #91 — token de autenticação
        },
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-8">
        <div className="bg-white rounded-2xl border border-green-200 p-8 text-center max-w-md w-full shadow-sm">
          ...
          <p className="text-gray-600 mb-2">
            A tua resposta foi registada com sucesso.
          </p>
          {authUser?.name && (
            <p className="text-sm text-gray-500 mb-6">
              Submetido por <span className="font-semibold">{authUser.name}</span>
            </p>
          )}
          <button
            onClick={onGoHome}
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-8 py-6 mb-6">
          ...
          {authUser?.name && (
            <p className="text-xs text-gray-400 mt-3">
              A preencher como <span className="font-semibold text-gray-600">{authUser.name}</span>
            </p>
          )}
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex gap-3 justify-end">
            <button
              type="button"
              onClick={onGoHome}
              className="px-6 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition font-semibold"
            >
              Cancelar
            </button>
            <button
              ...
              className="px-8 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition font-semibold disabled:opacity-60 disabled:cursor-not-allowed"
            >
