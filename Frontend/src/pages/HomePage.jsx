  ...
  useEffect(() => {
    const fetchTemplates = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Erro ao carregar formulários: ${response.statusText}`);
        }

        const allTemplates = await response.json();

        setDrafts(allTemplates.filter((t) => t.status === 'draft'));
        setPublished(allTemplates.filter((t) => t.status === 'published'));
      } catch (err) {
      ...
      <div className="max-w-7xl mx-auto mb-10">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-6 py-5 flex flex-col gap-2">
          ...
          <p className="text-gray-500">
            Cria novos formulários, continua rascunhos ou preenche formulários publicados.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        {loading && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mt-4">A carregar formulários...</p>
          </div>
        )}
        ...
        {/* ======================================================
            FORMULÁRIOS PUBLICADOS (#104 — navegação consolidada)
        ====================================================== */}
        {!loading && !error && (
          <section>
            <div className="mb-6">
            ...
            {published.length === 0 ? (
              <div className="bg-white rounded-lg p-10 text-center border border-gray-200">
                ...
                <p className="text-gray-500">Nenhum formulário publicado ainda</p>
                <p className="text-gray-400 text-sm mt-1">
                  Publica um formulário no editor para aparecer aqui
                </p>
              </div>
            ) : (
                ...
                {published.map((form) => (
                  ...
                  >
                    <div className="bg-gradient-to-r from-green-50 to-emerald-50 px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                      ...
                      <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full font-semibold shrink-0 ml-2">
                        Publicado
                      </span>
                    </div>
                    ...
                    <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
                      <button
                        ...
                        className="w-full px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-semibold text-sm text-center"
                      >
        ...
        {!loading && !error && (
          <section>
            <div className="mb-6">
              ...
              <p className="text-gray-500 text-sm">
                Formulários guardados que pode continuar a editar
              </p>
            </div>
            ...
            {drafts.length === 0 ? (
              <div className="bg-white rounded-lg p-10 text-center border border-gray-200">
                ...
                <p className="text-gray-400 text-sm mt-1">
                  Começa criando um novo formulário
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
